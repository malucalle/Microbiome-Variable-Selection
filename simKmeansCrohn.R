############################################################################
#   SIMULATION BASED ON CROHN MICROBIOME DATA (with Kmeans)
#
...
dirWrite=paste('./ItrKmeans',namefsimul,sep="");
dir.create(dirWrite);
...
#############################################################################
# simulation according read parameters
#############################################################################
...
for(k in (1:niter)){ 
    ...
    Xtrue<-X[,1:ntrue]   # we may reduce the number of individuals
    ...
    # Matrix of proportions
    Xtrue<-Xtrue/rowSums(Xtrue)
    
    # CLR transformation Z=log(X)
    z1true<-log(Xtrue)
    clrztrue<- apply(z1true,2,function (x) x-rowMeans(z1true))
    
    #kmeans
    
    cl<-kmeans(clrztrue,2)
    y<-as.numeric(cl$cluster)-1
      ...
      Xselbal=X;
        ...
        # selbal_sim<-selbal_simulations(x = X, y = y_selbal, logt=T, maxV=ncol(X), th.imp=0, draw=F)
        ...
        denom[is.na(denom)]=0; #::TONI:: Check This!!!!
